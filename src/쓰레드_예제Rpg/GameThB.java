    ...
    public void run() {
        ...
        while(true) { // 해당 쓰레드를 무한 반복하도록 만듬
            try {
                ...
                if(normal == 1) {
                    System.out.println(Character.FONT_BLUE + "마법 공격 : " + warrior.name + " 에게" +
                            wizard.mAttack() + " 데미지를 입혔습니다." + Character.RESET);
                    endGame  = warrior.setDamage(wizard.mAttack());
                } else {
                    System.out.println(Character.FONT_BLUE + "물리 공격 : " + warrior.name + " 에게" +
                            wizard.pAttack() + " 데미지를 입혔습니다." + Character.RESET);
                    endGame  = warrior.setDamage(wizard.pAttack());
                }

                if(special == 10) { // 1/18 확률로 발동
                    System.out.println(Character.BACKGROUND_WHITE + Character.FONT_BLUE + wizard.name + " 궁극기 발동 !!!!, "
                            + warrior.name + "에게" + wizard.ultimate()
                            + "데미지를 입혔습니다." + Character.RESET);
