        ...
        while(true) { // 해당 쓰레드를 무한 반복하도록 만듬
            try {
                ...
                if(normal == 1) { // normal 은 0/1 만 있음
                    System.out.println(Character.FONT_RED +"물리 공격 : " + wizard.name + "에게"
                            + warrior.pAttack() + "데미지를 입혔습니다." + Character.RESET);
                    endGame = wizard.setDamage(warrior.pAttack());
                } else {
                    System.out.println(Character.FONT_RED +"마법 공격 : " + wizard.name + "에게"
                            + warrior.mAttack() + "데미지를 입혔습니다."+ Character.RESET);
                    endGame = wizard.setDamage(warrior.mAttack());
                }
                if(special == 10) { // 10이라는 값이 나올 확률은 1/20 이다.
                    System.out.println(Character.BACKGROUND_WHITE + Character.FONT_RED + warrior.name + " 궁극기 발동 !!!, "
